...
variable "deployment_type" {
  description = "The filesystem deployment type. One of: `MULTI_AZ_1`, `MULTI_AZ_2`, `SINGLE_AZ_1`, or `SINGLE_AZ_2`"
  type        = string
  default     = "MULTI_AZ_1"

  validation {
    condition     = contains(["MULTI_AZ_1", "MULTI_AZ_2", "SINGLE_AZ_1", "SINGLE_AZ_2"], var.deployment_type)
    error_message = "deployment_type must be one of: MULTI_AZ_1, MULTI_AZ_2, SINGLE_AZ_1, SINGLE_AZ_2."
  }
}
...
variable "file_system_id" {
  description = "ID of an existing FSx ONTAP file system to look up when create = false"
  type        = string
  default     = null
}

variable "storage_virtual_machine_ids" {
  description = "Map of logical key to SVM ID for existing SVMs to look up when create = false. Keys should match those used in storage_virtual_machines"
  type        = map(string)
  default     = {}
}
...
variable "volume_backups" {
  description = "Map of on-demand backup definitions. Each entry creates an aws_fsx_backup for an ONTAP volume. Specify volume_key (composite 'machine_key_volume_key' for a module-managed volume) or volume_id (for an externally-managed volume)"
  type = map(object({
...
variable "volume_backups_timeouts" {
  description = "Create and delete timeout configurations for volume backups"
  type        = map(string)
  default     = {}
}
