...
resource "aws_fsx_ontap_volume" "this" {
  ...
  final_backup_tags                    = try(each.value.final_backup_tags, null)
...
################################################################################
# Data Source Lookups
# Used when create = false to reference existing resources not managed here
################################################################################

data "aws_fsx_ontap_file_system" "this" {
  count = !var.create && var.file_system_id != null ? 1 : 0

  id = var.file_system_id
}

data "aws_fsx_ontap_storage_virtual_machine" "this" {
  for_each = !var.create ? var.storage_virtual_machine_ids : {}

  id = each.value
}

################################################################################
# Unified locals — outputs reference these regardless of create vs lookup
################################################################################

locals {
  file_system_output = (
    var.create
    ? try(aws_fsx_ontap_file_system.this[0], null)
    : try(data.aws_fsx_ontap_file_system.this[0], null)
  )

  storage_virtual_machines_output = (
    var.create
    ? aws_fsx_ontap_storage_virtual_machine.this
    : data.aws_fsx_ontap_storage_virtual_machine.this
  )
}
...
resource "aws_fsx_backup" "this" {
  ...
  volume_id = try(
    aws_fsx_ontap_volume.this[each.value.volume_key].id,
  ...
  timeouts {
    create = try(var.volume_backups_timeouts.create, null)
    delete = try(var.volume_backups_timeouts.delete, null)
  }
}
